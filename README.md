...
```txt
/www/wwwroot/srf.cccz.cc/data/records/账号/时间.md
```
...
第一次使用点“注册”，以后点“登录”。登录成功后窗口会自动隐藏到右下角托盘。
...
同一个账号可以在多台 Windows 电脑上同时登录。手机端发送后，所有同账号电脑都会收到消息，但只有当前输入框内有光标的那台电脑会自动粘贴，其它电脑不会有反应。
...
先把目标电脑光标点到 VS Code、Cursor、记事本、微信输入框等可输入区域。多台电脑同账号在线时，只有当前有输入光标的电脑会响应。某些管理员权限窗口可能需要用相同权限启动客户端。
