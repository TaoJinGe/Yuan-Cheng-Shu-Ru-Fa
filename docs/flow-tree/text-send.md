...
状态变化：服务端记录文本并向同账号所有桌面端发送 insert_text，记录文件按日期创建并按发送时间分段追加；桌面端有活动窗口时写剪贴板并 Ctrl+V，现代应用即使检测不到传统输入光标也会尝试粘贴；粘贴成功只更新客户端状态文字，不弹出系统通知
关联功能：AUTH_LOGIN_001、PRESENCE_SYNC_001
风险等级：高
验证方式：多台电脑同账号登录，当前输入框内有光标的电脑在手机发送后出现文本；VS Code、Cursor、微信等现代输入框也应能粘贴
