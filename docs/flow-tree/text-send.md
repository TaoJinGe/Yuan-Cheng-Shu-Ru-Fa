...
状态变化：手机端按下发送按钮时稳定触发一次发送，发送按钮贴近屏幕底部或输入法上沿，发送后保存上一条输入内容并自动清空输入框，顶部撤销按钮可恢复上一条继续编辑；按钮提供按下和发送闪烁反馈，并只把 message_type 为 send_text 的 ack 当作投递结果；服务端记录文本并向同账号所有桌面端发送 insert_text，记录文件按日期创建并按发送时间分段追加；桌面端有活动窗口时写剪贴板并用 keybd_event 发送 Ctrl+V，剪贴板恢复延迟 5 秒，客户端只提示已发送粘贴指令，不弹出系统通知
