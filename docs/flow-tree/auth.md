...
数据影响：SQLite app.db users 表和 sessions 表
状态变化：保存 bcrypt 密码哈希，生成 24 小时或 30 天 token，并持久化 session
...
数据影响：SQLite app.db sessions 表、服务端内存 sessions
状态变化：吊销 token，本地删除 token，服务端删除持久化 session
